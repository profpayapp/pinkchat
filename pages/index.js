...
export default function App() {
  ...
  useEffect(() => {chatEndRef.current?.scrollIntoView({ behavior: "smooth" })}, [chats, activeContact, typing, generatingImage])
  ...
  const sendMessage = async (imageUrl = null, audioUrl = null, videoUrl = null, docName = null) => {
    ...
    if(groupMode) {
      Object.keys(contacts).forEach((name, index) => {
        setTimeout(() => {setTyping(name); setTimeout(() => {setTyping(null); aiReplyGroup(name, input)}, 1500 + index * 500)}, index * 2000)
      })
    } else {
      setTyping(activeContact)
      setTimeout(() => {setTyping(null); aiReply(activeContact, input, imageUrl, audioUrl, videoUrl, docName)}, 1500)
    }
  }
  ...
  const aiReply = async (name, msg, imageUrl, audioUrl, videoUrl, docName) => {
    ...
    if(imageUrl) return addAiMsg(name, "Wow you created this? 😍 It's beautiful!")
  ...
  return (
    <div style={{background: bgColor, color: textColor, minHeight: "100vh", padding: "10px"}}>
      ...
      <div style={{background: chatBg, padding: "12px", borderRadius: "10px", height: "50vh", overflowY: "auto"}}>
        ...
        {generatingImage && <div style={{fontSize: "12px", opacity: 0.7}}>🎨 Generating image...</div>}
        <div ref={chatEndRef} />
      </div>
      ...
      <div style={{marginTop: "8px"}}>
        <div style={{display: "flex", gap: "8px", marginBottom: "8px"}}>
          <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key==="Enter" && sendMessage()}
            placeholder={groupMode? "Message Group... or 'draw a cat'" : "Message or 'draw a cat'"} style={{flex: 1, padding: "12px", borderRadius: "25px", border: "none", outline: "none", fontSize: "14px"}}/>
