...
export default function Home() {
  ...
  const autoReplies = {
    Luna: ["aww 🥰", "hehe ok", "you're so sweet 💕", "tell me more!"],
    Coral: ["yo what's up", "bet", "fr??", "say less 😎"],
    Indigo: ["I hear you", "that's deep", "I'm here for you", "facts 💯"]
  }
  ...
  const sendMessage = () => {
    ...
    setTimeout(() => {
      const replies = autoReplies[activeContact]
      const randomReply = replies[Math.floor(Math.random() * replies.length)]
      const replyMsg = {text: randomReply, time: new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}), sender: "them"}
