...
export default function Home() {
  ...
  const [newContactPersonality, setNewContactPersonality] = useState("Sweet") // NEW
  const messagesEndRef = useRef(null)
  
  const [contacts, setContacts] = useState({
    Luna: {color: "#ff69b4", img: "https://i.pravatar.cc/150?img=1", personality: "Sweet"},
  ...
  useEffect(() => {scrollToBottom()}, [chats, activeContact])
  ...
  // UPDATED: PERSONALITY SYSTEM
  const getSmartReply = (contact, userMsg) => {
    const msg = userMsg.toLowerCase().replace(/["'.,!?]/g, "")
    const personality = contacts[contact].personality
    
    if(personality === "Sweet") {
      if(msg.includes("sad")) return "come here, I got you. You deserve better 💖"
      if(msg.includes("love")) return "aww I love you too 🥰"
      return ["you're so sweet", "tell me more", "that made me smile"][Math.floor(Math.random()*3)]
    }
    if(personality === "Chill") {
      if(msg.includes("ok")) return "yeah I'm good bro, just chilling. you?"
      return ["bet", "say less", "no cap"][Math.floor(Math.random()*3)]
    }
    if(personality === "Wise") {
      if(msg.includes("problem")) return "Let's think about this together. What's really bothering you? 💜"
      return ["I hear you", "That makes sense", "Have you considered..."][Math.floor(Math.random()*3)]
    }
    if(personality === "Motivator") {
      if(msg.includes("tired")) return "Champions don't quit. Rest 10min then get back to work 👑"
      return ["Let's go", "Lock in", "Execute now 💼"][Math.floor(Math.random()*3)]
    }
    if(personality === "Funny") {
      return ["lmaooo", "dead 💀", "you wildin", "stoppp 😂"][Math.floor(Math.random()*4)]
    }
    if(personality === "Therapist") {
      return ["I understand how that feels", "That sounds tough. Do you want to talk about it?", "I'm here for you"][Math.floor(Math.random()*3)]
    }
    if(personality === "Study") {
      return ["Let's break this down step by step", "Great question! The answer is...", "Want me to quiz you on this? 📚"][Math.floor(Math.random()*3)]
    }
  ...
  // UPDATED: ADD CONTACT WITH PERSONALITY
  const addContact = () => {
    if(newContactName.trim() === "" || contacts[newContactName]) return
    const colors = ["#00bfff", "#32cd32", "#ff1493", "#ffa500", "#9370db", "#ff69b4"]
    const randomColor = colors[Math.floor(Math.random()*colors.length)]
    const randomImg = `https://i.pravatar.cc/150?img=${Math.floor(Math.random()*70)}`
    
    setContacts(prev => ({
    ...prev,
      [newContactName]: {color: randomColor, img: randomImg, personality: newContactPersonality}
    }))
    setChats(prev => ({
    ...prev,
      [newContactName]: [{text: `Hi! I'm ${newContactName}. I'm your ${newContactPersonality} friend 😊`, time: new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}), sender: "them"}]
    }))
  ...
  return (
    <div style={{background: bgColor, color: textColor, minHeight: "100vh", padding: "20px", transition: "all 0.3s"}}>
      ...
      {/* UPDATED MODAL WITH PERSONALITY DROPDOWN */}
      {showAddModal && (
        <div style={{background: buttonBg, padding: "15px", borderRadius: "10px", margin: "10px 0"}}>
          <p><b>Add New Contact</b></p>
          <input value={newContactName} onChange={(e) => setNewContactName(e.target.value)} placeholder="Enter name: Bestie, Coach, etc" style={{padding: "8px", borderRadius: "8px", border: "1px solid #ddd", marginRight: "5px", width: "60%", marginBottom: "10px"}} />
          <br/>
          <label>Personality: </label>
          <select value={newContactPersonality} onChange={(e) => setNewContactPersonality(e.target.value)} style={{padding: "8px", borderRadius: "8px", marginRight: "5px"}}>
            <option>Sweet</option>
            <option>Chill</option>
            <option>Wise</option>
      ...
      <div style={{display: "flex", alignItems: "center", gap: "10px", margin: "10px 0"}}>
        ...
        <h2 style={{margin: 0}}>{activeContact} <span style={{fontSize: "12px", opacity: 0.7}}>({contacts[activeContact].personality})</span></h2>
      </div>
