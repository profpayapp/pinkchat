...
export default function App() {
  ...
  const sendMessage = () => {
    ...
    setInput("")
    setTimeout(() => {
      if(activeContact === "Group") groupReplyWithTyping()
      else aiReply(activeContact)
    }, 1000)
  }
  ...
  const sendGift = (gift) => {
    ...
    setTimeout(() => {
      ...
      const reactions = {"❤️": "Thank you for the heart! ❤️","🌹": "Wow a rose! 🌹","🔥": "This is fire! 🔥","🚀": "To the moon! 🚀","👑": "King behavior! 👑"}
  ...
  const toggleLive = async () => {
    if(!isLive) {
      ...
      setViewerCount(0)
      const time = new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})
      const msg = {text: `📹 Live ended • ${viewerCount} viewers • Total earned: ${coins} coins`, time, sender: user}
      setChats(prev => ({...prev, Group: [...prev.Group, msg]}))
    }
  }
  ...
  const playLastAudio = () => {if(lastAudioRef.current) new Audio(lastAudioRef.current).play()}
  ...
  if(!user) {
    return (
      <div style={{background: bgColor, color: textColor, minHeight: "100vh", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", padding: "20px"}}>
        ...
        <input value={profile.name} onChange={e => setProfile({...profile, name: e.target.value})} placeholder="Enter Username" style={{padding: "12px", borderRadius: "10px", border: "2px solid #ff69b4", margin: "8px 0", width: "80%", background: "#222", color: "#fff"}}/>
        <input value={profile.bio} onChange={e => setProfile({...profile, bio: e.target.value})} placeholder="Bio" style={{padding: "12px", borderRadius: "10px", border: "2px solid #ff69b4", margin: "8px 0", width: "80%", background: "#222", color: "#fff"}}/>
        <button onClick={handleLogin} style={{background: "linear-gradient(90deg, #ff69b4, #ffa500)", color: "#fff", border: "none", padding: "14px 40px", borderRadius: "25px", fontWeight: "bold", fontSize: "16px", marginTop: "10px"}}>Join PINKCHAT</button>
      </div>
    )
  }

  return (
    <div style={{background: bgColor, color: textColor, minHeight: "100vh", padding: "10px", paddingBottom: "90px"}}>
      ...
      <div style={{background: "linear-gradient(90deg, #ff69b4, #ffa500)", padding: "10px", borderRadius: "15px", marginBottom: "10px", textAlign: "center"}}>
        ...
        <p style={{color: "#fff", fontSize: "10px", margin: "0"}}>by CRYPTO-PROF | AI App Owner</p>
        <div style={{color: "#ffd700", fontWeight: "bold", marginTop: "5px"}}>🪙 Wallet: {coins} coins</div>
        <button onClick={() => setShowLeaderboard(!showLeaderboard)} style={{background: "#ffd700", color: "#000", border: "none", padding: "4px 10px", borderRadius: "10px", marginTop: "5px", fontSize: "11px"}}>🏆 Leaderboard</button>
        <button onClick={withdraw} style={{background: "green", color: "#fff", border: "none", padding: "4px 10px", borderRadius: "10px", marginTop: "5px", marginLeft: "5px", fontSize: "11px"}}>💰 Withdraw</button>
      </div>
      ...
      {isLive && (
        <div style={{background: "#000", borderRadius: "10px", marginBottom: "10px", padding: "5px"}}>
          ...
          <div style={{color: "red", textAlign: "center", fontWeight: "bold", fontSize: "14px"}}>🔴 LIVE • {viewerCount} viewers 👁️</div>
          <div style={{display: "flex", gap: "12px", justifyContent: "center", margin: "10px 0", background: "#1a1a1a", padding: "8px", borderRadius: "10px", flexWrap: "wrap"}}>
            {giftList.map(gift => (
              <button key={gift.name} onClick={() => sendGift(gift)} style={{background: "linear-gradient(90deg, #ff69b4, #ffa500)", border: "none", borderRadius: "50%", width: "50px", height: "50px", fontSize: "24px"}}>
      ...
      <div style={{margin: "8px 0", display: "flex", gap: "6px", flexWrap: "wrap"}}>
        {contacts.map(name => (
          <button key={name} onClick={() => setActiveContact(name)} style={{background: activeContact===name? contactColors[name] : "#333", color: "#fff", border: "none", padding: "6px 12px", borderRadius: "15px", fontSize: "12px", fontWeight: name==="Group"?"bold":"normal"}}>
      ...
      <div style={{background: chatBg, padding: "12px", borderRadius: "10px", height: "45vh", overflowY: "auto"}}>
        ...
        {chats[activeContact].map((m, i) => (
          <div key={i} style={{textAlign: m.sender===user? "right" : "left", margin: "8px 0"}}>
            ...
            {m.video? (
              ...
              <span onClick={() => m.hasAudio && playLastAudio()} style={{background: m.sender===user? "linear-gradient(90deg, #ff69b4, #ffa500)" : "#444", color: "#fff", padding: "8px 12px", borderRadius: "15px", display: "inline-block", maxWidth: "75%", cursor: m.hasAudio? "pointer" : "default"}}>
                {m.text}
              </span>
            )}
        ...
        {typing && <div style={{fontSize: "11px", color: "#ff69b4", fontStyle: "italic"}}>{typing}...</div>}
        <div ref={chatEndRef} />
      </div>

      <div style={{marginTop: "8px", display: "flex", gap: "8px"}}>
        <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key==="Enter" && sendMessage()} placeholder="Message..." style={{flex: 1, padding: "12px", borderRadius: "25px", border: "none", outline: "none"}}/>
      ...
      <div style={{display: "flex", gap: "10px", justifyContent: "space-around", marginTop: "10px"}}>
        ...
        <button onClick={toggleRecording} style={{background: recording? "red" : "linear-gradient(90deg, #ff69b4, #ffa500)", border: "none", color: "#fff", fontSize: "11px", padding: "8px 14px", borderRadius: "20px", fontWeight: "bold"}}>🎤 {recording? "Stop" : "Talk"}</button>
        <button onClick={toggleLive} style={{background: isLive? "red" : "none", border: "none", color: isLive? "#fff" : textColor, fontSize: "10px", fontWeight: isLive? "bold" : "normal"}}>📹 {isLive? "End Live" : "Live"}</button>
      </div>
    </div>
  )
}
