  ...
  return (
    ...
    >
      ...
      <div style={{display: "flex", justifyContent: "center", gap: "20px"}}>
        <button onClick={toggleMic} style={{width: "60px", height: "60px", borderRadius: "50%", background: "#fff", border: "none"}}>
          {isMuted? <MicOff color="#FF1493" /> : <Mic color="#FF1493" />}
        </button>
        <button onClick={onClose} style={{width: "60px", height: "60px", borderRadius: "50%", background: "red", border: "none"}}>
          <PhoneOff color="#fff" />
        </button>
      </div>
    </motion.div>
  ...
  const toggleRecording = async () => {
    if(!recording) {
      ...
      mediaRecorderRef.current.onstop = () => {
        ...
        lastAudioRef.current = URL.createObjectURL(audioBlob)
        const time = new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})
        setChats(prev => ({...prev, [activeContact]: [...prev[activeContact], {text: `🎤 Voice Note ▶️`, time, sender: user, hasAudio: true}]}))
      }
  ...
  return (
    <div style={{background: bgColor, color: textColor, minHeight: "100vh", padding: "10px", paddingBottom: "80px"}}>
      ...
      <div style={{margin: "8px 0", display: "flex", gap: "6px", flexWrap: "wrap"}}>
        {["Group", "Prof", "Queen", "Indigo", "Boss", "Tech", "Gist"].map(name => (
          <button key={name} onClick={() => setActiveContact(name)} style={{background: activeContact===name? "#FF1493" : "#333", color: "#fff", border: "none", padding: "6px 12px", borderRadius: "15px"}}>{name}</button>
        ))}
      </div>
      ...
      <div style={{background: chatBg, padding: "12px", borderRadius: "10px", height: "50vh", overflowY: "auto"}}>
        {chats[activeContact].map((m, i) => (
          <div key={i} style={{textAlign: m.sender===user? "right" : "left", margin: "8px 0"}}>
            <span style={{background: m.sender===user? "#FF1493" : "#444", color: "#fff", padding: "8px 12px", borderRadius: "15px"}}>{m.text}</span>
          </div>
        ))}
        <div ref={chatEndRef} />
      </div>
